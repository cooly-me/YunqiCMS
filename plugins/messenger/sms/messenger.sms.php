...
//  var $targetSplit      = ','; //多目标分隔符
    ...
    function send($to,$message,$config,$sms_type){
        ...
        $send_type = ('notice' == $sms_type) ? 'notice' : 'fan-out'; // todo 
        return $this->sms->send('['.json_encode($content).']'.$reply,$send_type);
    }
    ...
    function extraVars(){
        return array('outgoingOptions'=>$this->sms->getSmsUrl('sms','charge'));
    }
}
