...
class ctl_passport extends shopPage{
    ...
    function verifyCode($type=''){
        ...
        $code = $oVerifyCode->init(4);
        if( !empty($type) ){
            $sessionName = strtoupper($type).'_RANDOM_CODE';
        }else{
            $sessionName = 'RANDOM_CODE';
        }
    ...
    function splash($status='fail', $urlo=null, $msgo=null, $links='', $wait=3, $json=false){
        if($json){
    ...
    function create() {
        ...
        // 检查验证码        
        if($this->system->getConf('site.register_valide')){       
            if(strtolower($_POST["signupverifycode"]) !== strtolower($_SESSION["S_RANDOM_CODE"])){
    ...
    function verify() {
        $_POST['ref_url'] = strip_tags($_POST['ref_url']); //过滤掉URL中的htmL
        $memberObj = &$this->system->loadModel('member/account');
        // 检查验证码        
        if($this->system->getConf('site.login_valide')){            
            if(strtolower($_POST["loginverifycode"]) !== strtolower($_SESSION["L_RANDOM_CODE"])){
