...
class ctl_payment extends objectPage {
    ...
    */
    ...
    function index($params1,$params2){
        ...
        foreach($allApp as $k => $v){
            if($v['pay_ident']=='pay_teegonali' || $v['pay_ident']=='pay_teegonwxpay'){
                unset($allApp[$k]);
            }
        }
        ...
        $this->pagedata['yq_account_url'] = YUNQI_ACCOUNT_URL;
    ...
    */
    function editPayment($ident){
        ...
        if( !$payPlugin = $payMdl->loadMethod($paytype) ) {
            trigger_error("支付方式：{$ident}不存在", E_USER_ERROR);
        }

        $conf = $payMdl->getPaymentByIdent($paytype);
