...
class ctl_certificate extends adminPage{
    ...
    function showIndex(){
        ...
        $this->ent_ac = $this->certi_model->getent_ac();
        ...
        $this->pagedata['ent_ac'] = $this->ent_ac;
    ...
    function del(){
        ...
        $this->certi_model->delEnterprise();   //删除证书清除激活信息

        $this->clear_all_cache();

        $opt = $this->system->loadModel('admin/operator');
        $url = $opt->logout('step_active',$error_msg);
    ...
    function node_binding($subact='accept') {
        ...
        $data['source'] = $subact;
        $data['callback'] = rawurlencode($this->system->base_url().'?tools-callback.html');
        $data['api_v'] = '2.0';
        $data['from_api_v'] = '2.0';
        $data['to_api_v'] = '2.2';
