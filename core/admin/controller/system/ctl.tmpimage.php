...
class ctl_tmpimage extends objectPage{
    ...
    function show_detail($sName) {
        ...
        if($file['filetype'] == "css"||$file['filetype'] == "html"||$file['filetype'] == "xml"||$file['filetype'] == "js"||$file['filetype'] == "txt"){
    ...
    function saveImage(){
        ...
        if( false === strpos($_POST['id'],$_POST['name']) ){
            $this->end(false, __('保存文件名不合法'));
        }
    ...
    function saveSource(){
        ...
        $extend = pathinfo($_POST['name'],PATHINFO_EXTENSION);
        if( !check_file_name($extend) || (false === strpos($_POST['id'],$_POST['name']))){
            $this->end(false, __('保存文件名不合法'));
        }
