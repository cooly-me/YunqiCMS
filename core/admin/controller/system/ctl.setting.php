...
class ctl_setting extends adminPage{
    ...
    function welcome(){
        $this->pagedata['remove_ads'] = $this->system->getConf('system.remove_ads');
    ...
    function settingEdit(){
        $order_canceltime = $_POST['setting']['system.order_canceltime'];
        if($order_canceltime){
            $oOrder = $this->system->loadModel('trading/order');
            $oOrder->setModifiedtime($order_canceltime);

            //注册后台定时更新订单状态
            $appmgr = &$this->system->loadModel('system/appmgr');
            $appmgr->register_crontab_queue('trading/order:run_system_queue');
        ...
        //绑定物流跟踪 start
        if( $_POST['setting']['site.logistics.track'] ){
            if( ! $this->system->getConf('certificate.yunqi_token') ){
                trigger_error(__('物流信息激活失败'),E_USER_ERROR);
            }
        }
        //绑定物流跟踪 end
    ...
    function watermark(){
        ...
        if($loaded){
            ...
            $this->pagedata['wm_small_text_transition'] = $this->system->getConf('site.watermark.wm_small_text_transition');
            ...
            $this->pagedata['wm_big_text_transition'] = $this->system->getConf('site.watermark.wm_big_text_transition');
        ...
        if($loaded){
            ...
            if($watermark['wm_small_enable']){
                ...
                if($watermark['wm_small_enable']==1){
                    ...
                    $watermark['wm_small_text_transition']=$data['wm_small_text_transition'];

                }
            }else{
                $watermark['wm_small_pic'] = '';
            }
            ...
            if($watermark['wm_big_enable']){
                ...
                if($watermark['wm_big_enable']==1){
                    ...
                    $watermark['wm_big_text_transition']=$data['wm_big_text_transition'];

                }
            }else{
                $watermark['wm_big_pic'] = '';
            }
    ...
    function watermarkPreview($tag){
        ...
        $ib->wm_text_transition = $this->system->getConf('site.watermark.wm_'.$tag.'_text_transition');
    ...
    function createPreviewImg($tag, $upload=false){
        ...
        if($upload){
            ...
            $preview = $pic ? '../images/default/default_preview_pic.jpg' : '../images/default/shopexstorelogo.jpg';
...
echo '<div align=center style="padding:10px;"><img border=1 src="'.$preview.'?'.time().'" width='.$_POST[$tag.'_pic_width'].' height='.$_POST[$tag.'_pic_height'].'/></div></div></div>';
echo '</form>';
        }
        else{
            $ib=&$this->system->loadModel('utility/magickwand');
            ...
            $ib->wm_text_transition = $_POST['wm_'.$tag.'_text_transition'];
    ...
    function centerSend($info,$params){
        ...
        foreach($params as $key=>$value){
            ...
            if(substr($key,0,5)=='store'){
                //商店网址后缀必须"/"结束,防止与中心通信错误
                if(substr($key,6) == 'shop_url' && substr($value,-1) != '/'){
                    $value = $value.'/';
                }
    ...
    function greencard(){
        ...
        $params = array(
            'client_id' => OAUTH_KEY,
            'license_id' => $cert->getCerti()          
        );
        $params['ac'] = $cert->make_shopex_ac($params,$cert->getToken());
    ...
    function serviceMarket(){
        $oCertificate = $this->system->loadModel('service/certificate');
        $source = $oCertificate->iframe_source_encode('485');
        $url = YUNQI_SERVICE_URL.'cid=49&source='.$source;
        echo "<iframe frameborder='0' allowtransparency='true' scrolling='auto' width='100%' height='100%' src=".$url."></iframe>";exit;
    }

    function logisticTracking(){
        $oCertificate = $this->system->loadModel('service/certificate');
        $source = $oCertificate->iframe_source_encode('485');
        $url = YUNQI_LOGISTIC_URL.'?ctl=exp&act=index&source='.$source;
        echo "<iframe frameborder='0' allowtransparency='true' scrolling='auto' width='100%' height='100%' src=".$url."></iframe>";exit;
    }
    ...
    * 短信平台 iframe
    */
    function smsResource(){
        $oCertificate = $this->system->loadModel('service/certificate');
        $data[] = base64_encode(SOURCE_ID);
        $data[] = $oCertificate->getent_id();
        $data[] = $oCertificate->getyunqi_token();
        $data[] = time();
        $data[] = $oCertificate->getRandChar(6);
        $data[] = $oCertificate->getRandChar(6);
        $source_str = implode('|', $data);
        $this->pagedata['resource_url'] = SMS_USER_API_URL . 'index.php?source='.base64_encode($source_str);
        $this->page('setting/sms_resource.html');
    }
}
