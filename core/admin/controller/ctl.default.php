...
class ctl_default extends adminPage{
    ...
    function index($params1,$params2){
        ...
        if(!IN_AJAX){
            ...
            $this->pagedata['authcode'] = $this->system->getConf('certificate.auth_code');
            $this->pagedata['authsingle'] = $certMdl->getUrl('','single');
    ...
    function status(){
        ...
        foreach(unserialize($this->system->getConf("system.crontab_queue")) as $k =>$v ){
            ...
            if($method_count == 2){
                ...
                list($objCtl,$act_method) = $appmgr->get_func($v);//app cron_tab
                if(method_exists($objCtl,$act_method)){
                    $objCtl->$act_method();
                }
            }
    ...
    function getcertidandurl(){
        ...
        if(!strstr($cet_ping,'HTTP/1.1 200 OK')){
            ...
            $center_url = "http://guide.ecos.shopex.cn/index.php?certi_id=".$cert_id.'&url='.urlencode($base_url).'&confirmkey='.$confirmkey.'&sess_id='.$sess_id.'&auth_code='.$auth_code;
            echo $center_url;
        }
    }
