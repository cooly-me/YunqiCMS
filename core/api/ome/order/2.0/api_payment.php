...
class api_payment extends shop_api_object {
    ...
    function dopay($payment){
        ...
        // 检查支付方式
        //$payment['payment'] || $payment['payment'] = $payment['pay_type'];
        $where = '';
        if($payment['payment']){
            $where = "WHERE id=".intval($payment['payment']);
        }elseif($payment['pay_type']){
            $where = "WHERE pay_type='".$payment['pay_type']."'";
        }else{
            $where = "WHERE id=".intval($payment['payment']);
        }
        $payment_cfg = $this->db->selectrow('SELECT id,pay_type,custom_name FROM sdb_payment_cfg '.$where);
        ...
        if( 'deposit' == $payment_['pay_type'] ){
            $message = '预存款支付：订单号{'.$payment['order_id'].'}';
            $payment_id = $payment_['payment_id'];
            $order_id = $payment['order_id'];
            $paymethod = $payment['paymethod'];
            $memo = $payment['memeo']!=''?$payment['memo']:$payment['paymethod'];
            if ( !$advanceMdl->deduct($buyer['member_id'],$payment_['money'],$message,&$errMsg, $payment_id, $order_id ,$paymethod,$memo) ) {
    ...
    function init_payment_cfg(){
        ...
        $this->api_response('true','data true',null,$cfgs);
