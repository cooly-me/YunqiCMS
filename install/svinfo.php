...
class mdl_serverinfo{
    ...
    function test_server_req(&$score){
        ...
        $items['MySQLi函数库可用'] = array(
            'value'=>$rst?mysqli_get_client_info():'未安装',
            'result'=>$rst,
        );
        if(!$rst){
            $allow_install = false;
            ...
            }elseif($db_ver = mysqli_get_server_info()){
                define('DB_HOST','');
            }else{
            ...
            $rst = $phpversion[0].$phpversion[2] >= 53 ? (function_exists('extension_loaded') && extension_loaded('Zend Guard Loader')) : (defined('OPTIMIZER_VERSION') || (function_exists('extension_loaded') && extension_loaded('Zend Optimizer')));
            if($rst){
            ...
            $zend = $phpversion[0].$phpversion[2] >= 53 ? 'Zend Guard Loader' : 'ZEND Optimizer2.5.7以上';
            $items[$zend] = array(
                'value'=>$value,
                'result'=>$rst,
            );
            ...
            if( $GLOBALS['system'] ){
                $r = $this->check_oauth_login();
                $items['ShopexID登陆接口'] = array(
                    'value'=> $r ? '通畅' : '不通',
                    'result'=> !!$r,
                );
                if(!$r){
    ...
    function check_oauth_login(){
        ...
        $params = array('callback' => urlencode($admin_url));
        $oauth_login['link_url'] = urlencode($admin_url);
        $oauth_login['ac'] = $certi_model->make_shopex_ac($params,ACTIVE_OAUTH_KEY);
        $login_url = AUTH_USER_URL.'?c=oauth&m=index&ac='.$oauth_login['ac'].'&callback='.$oauth_login['link_url'];
        return $toolsMdl->curl_call($login_url,'','GET');
    }
...
if(!defined('IN_INSTALLER')){
    ...
    /*- img-end -*/
        ...
        $ver = parse_ini_file(CORE_DIR.'/version.txt');
        $version = $ver['rev'];
